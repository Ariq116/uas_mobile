...
public class OrderHistoryAdapter extends RecyclerView.Adapter<OrderHistoryAdapter.ViewHolder> {
    ...
    public void onBindViewHolder(@NonNull ViewHolder holder, int position) {
        ...
        holder.tvNomorOrder.setText("Order " + order.getOrderId() + " - " + order.getTanggal());
        ...
        // ✅ Tentukan status otomatis berdasarkan metode pembayaran
        String status;
        if (order.getMetode() != null) {
            if (order.getMetode().equalsIgnoreCase("COD")) {
                status = "Belum dibayar";
            } else if (order.getMetode().equalsIgnoreCase("Transfer")) {
                status = "Diproses";
            } else {
